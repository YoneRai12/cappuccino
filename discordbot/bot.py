# discordbot/bot.py (パス問題を根本解決する最終確定版)
import os
import sys
import re

# ★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★
# ★★★ これがパス問題を解決する最も重要なコードです ★★★
# bot.pyが自分の親フォルダ(cappuccino)を見つけられるようにする
...
# 親フォルダをパスに追加したので、直接インポートできる
from poker import PokerMatch, PokerView

# ───────────────── TOKEN / KEY ─────────────────
ROOT_DIR = os.path.dirname(os.path.abspath(__file__))

# .envファイルはプロジェクトのルートにあると想定
    ...
    try:
        ...
        if isinstance(final_answer, str) and "画像を生成しました。パス: " in final_answer:
            ...
            if os.path.exists(path_str):
                ...
                response_text = f"エラー: 生成された画像ファイルが見つかりませんでした。パス: {path_str}"
        else:
...
async def on_message(message: discord.Message):
    ...
    if bot.user in message.mentions or (message.reference and message.reference.resolved and message.reference.resolved.author == bot.user):
        user_text = _strip_bot_mention(message.content)
        await handle_agent_request(message, user_text)

    if message.content.startswith("y?"):
        user_text = message.content[2:].strip()
        await handle_agent_request(message, user_text)
...
if __name__ == "__main__":
    if not TOKEN:
        raise RuntimeError("DISCORD_BOT_TOKEN is not set in .env or config file")
    try:
        bot.run(TOKEN)
